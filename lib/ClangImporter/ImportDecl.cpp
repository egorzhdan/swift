    ...
    Decl *VisitRecordDecl(const clang::RecordDecl *decl) {
      ...
      if (cxxRecordDecl) {
        ...
        for (const auto method : methods) {
          if (const auto clangDecl = method->getClangDecl()) {
            const auto cxxMethod = dyn_cast<clang::CXXMethodDecl>(clangDecl);
            if (cxxMethod &&
                cxxMethod->isOverloadedOperator() &&
                cxxMethod->getOverloadedOperator() == clang::OO_Subscript) {
              assert(method->getParameters()->size() == 1);

              auto parameterType = method->getParameters()->get(0)->getType();
              std::pair<NominalTypeDecl *, Type> key = { result, parameterType };
              if (parameterType &&
                  Impl.cxxSubscripts.find(key) != Impl.cxxSubscripts.end()) {

                auto getterAndSetter = Impl.cxxSubscripts[key];
                ...
                // That would happen for structs that have both const and
                // non-const `operator[]` for the same parameter type.
                Impl.cxxSubscripts.erase(key);
              }
            }
          }
        }
