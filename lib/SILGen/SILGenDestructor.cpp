...
void SILGenFunction::emitDestroyingDestructor(DestructorDecl *dd) {
  ...
  SILValue selfValue = emitSelfDeclForClassDeinit(dd->getImplicitSelfDecl());
  ...
  // A Swift class that subclasses a C++ foreign reference type does not chain to
  // the base's destructor: the imported FRT has no Swift deinit body, and its
  // storage is reclaimed through the FRT's custom release, not the Swift
  // destructor. (A transitive Swift subclass still chains to its Swift base.)
...
void SILGenFunction::emitDeallocatingClassDestructor(DestructorDecl *dd,
  ...
  SILValue initialSelfValue =
      emitSelfDeclForClassDeinit(dd->getImplicitSelfDecl());
...
void SILGenFunction::emitDeallocatingMoveOnlyDestructor(DestructorDecl *dd) {
  ...
  // Emit the prolog.
  auto selfValue = emitSelfDeclForMoveOnlyDeinit(dd->getImplicitSelfDecl());
...
void SILGenFunction::emitIVarDestroyer(SILDeclRef ivarDestroyer) {
  ...
  ManagedValue selfValue;
  {
    SILValue rawSelfValue =
        emitSelfDeclForClassDeinit(cd->getDestructor()->getImplicitSelfDecl());
